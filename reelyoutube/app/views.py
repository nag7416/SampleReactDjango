...
def get_user_data(request):
    ...
    if user.is_authenticated:
        user_videos = Video.objects.filter(author=user, visibility='Public').all()
...
def explore_videos(request):
    user = request.user
    videos = Video.objects.filter(visibility='Public').order_by('-views')[:100]
...
def user_channel_json(request, channelslug):
    channel = get_object_or_404(Channel, channelslug=channelslug)
    videos = Video.objects.filter(author=channel.channeluser, visibility='Public').all()
    admin_videos = Video.objects.filter(author=channel.channeluser).all()
    ...
    videos = [{
        ...
        'published': time_ago(video.published),
        'description': video.description,
        'likes': len(video.liked.all()),
        'comments': len(something.objects.filter(comment_video=video).all())
    } for video in videos ]
    ...
    admin_videos = [{
        ...
        'published': time_ago(video.published),
        'description': video.description,
        'likes': format_views_as_K(len(video.liked.all())),
        'comments': format_views_as_K(len(something.objects.filter(comment_video=video).all()))
...
def author_channel_json(request, channelslug):
    ...
    videos = Video.objects.filter(author=user.channeluser, visibility='Public').all()
...
def sub_videos(request):
    ...
    if request.user.is_authenticated:
        sub_channels = Channel.objects.filter(subscribers=request.user)
        sub_videos = Video.objects.filter(author__channeluser__in=sub_channels, visibility='Public').all()
...
def liked_videos_api(request):
    if request.user.is_authenticated:
        videos = Video.objects.filter(liked=request.user, visibility='Public').all().order_by('id')
        video_list = []
        for video in videos:
            video_dict = {
                'id': video.id,
...
def user_videos(request):
    if request.user.is_authenticated:
        videos = Video.objects.filter(author=request.user, visibility='Public').order_by('-published')
        video_list = []
        for video in videos:
            video_dict = {
                'id': video.id,
...
def featured_video(request):
    if request.user.is_authenticated:
        videos = Video.objects.filter(author=request.user, visibility='Public').order_by('-views').first()
        video_list = []
...
def library_videos_json(request):
    ...
    if request.user.is_authenticated:
        hisvideos = HistoryVideo.objects.filter(his_user=request.user).all().order_by('-his_time')
        watchvideos = Video.objects.filter(watchlater=user, visibility='Public').all()
        likedvideos = Video.objects.filter(liked=request.user, visibility='Public').all()
        playlists = Playlist.objects.filter(playlist_user=request.user).all()
...
def search_json(request):
    ...
    result = Video.objects.filter(Q(title__icontains=query) | Q(description__icontains=query), visibility='Public')
    users = Channel.objects.filter(channelname__icontains=query).all()
    vids = ''
    for user in users:
        if user in Channel.objects.filter():
    ...
    serialized_results = [{
        ...
        'image': 'http://localhost:8000'+video.image.url,
        'author': video.author.channeluser.channelname,
        'authorimage': 'http://localhost:8000'+video.author.channeluser.channelimg.url,
        'authorslug': video.author.channeluser.channelslug,
        'duration': video.duration,
        'views': format_views_as_K(video.views),
        'description': video.description,
